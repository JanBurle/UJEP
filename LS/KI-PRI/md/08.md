...
- S: Je nebo není bezpečná (neovlivní stav serveru).
- I: Je nebo není idempotentní (požadavek lze opakovat bez
  změny výsledku).
- C: Je nebo není kešovatelná (lze je uložit do mezipaměti po
  cestě).
...
```http
GET /api/users?key=value&key2=value2 HTTP/1.1
Host: example.com
```
