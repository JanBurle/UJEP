...
- časová náročnost (složitost)
  [O(?)](https://en.wikipedia.org/wiki/Time_complexity)
- stabilita (zachování pořadí stejných prvků)
- in-place (třídí na místě, v původním seznamu)
- paměťová náročnost (složitost)
  [O(?)](https://en.wikipedia.org/wiki/Space_complexity)
...
Algoritmus prochází seznam a vyměňuje sousední prvky, pokud
nejsou ve správném pořadí. Zde je naivní implementace:

(`qswap` je zkratka pro _question-swap_, vyměň prvky, pokud je
otázka "je první prvek větší než ten následující?" zodpovězena
kladně.)
...
Při psaní programů je vhodné, ba důležité až nezbytné testovat
jejich správnost. Nejlépe je psát testy současně s
implementací, a to ve formě testovacího programu.
...
Testovací program lze generalizovat a implementovat jako
třídu:
...
Není efektivní (je horší než bubble sort), ale zajímavý z
pohledu programování.
...
Vylepšený bubble sort, který eliminuje "želvy". Rychlejší než
bubble sort, ale nestabilní.
...
```python
def selectionSort(lst: list): # in-place
  def qswap(i: int, j: int):
    li,lj = lst[i],lst[j]
    if li > lj:
      lst[i], lst[j] = lj, li
...
def quickSort(lst: list):
  ...
  def partition(lo:int, hi:int):
    ...
    # swap(pivot_idx, hi - 1)
    # pivot: the rightmost item (naive choice)
    pivot = lst[hi-1]
...
Otestujte výše uvedené řadicí algoritmy. Změřte jejich časovou
náročnost a porovnejte výsledky.
...
Vytvořte třídu `Sort`, která bude mít metody pro výše uvedené
řadicí algoritmy. Seznam k setřídění bude zadán jako parametr
v konstruktoru třídy. Druhý parametr konstruktoru bude
volitelný, a bude se jím volit algoritmus, který se použije k
setřídění.

Upravte třídu `TestSort` tak, aby testovala všechny algoritmy
třídy `Sort`.

Upravte třídu `OrderedList`, její "línou" variantu z minulého
cvičení tak, aby použila třídu `Sort` k setřídění seznamu.

## ❖ Úloha

Analyzujte a vysvětlete, jak funguje `heapify()`.

## ❖ Úloha

Upravte (přepište) algoritmy tak, aby byl kód více pythonský
(pythonic).
