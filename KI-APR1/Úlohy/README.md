...
Typ `float` (_aproximace_ reálných čísel) má velký rozsah, ale je nepřesný. To je dáno tím, že doména čísel `float` není spojitá, ale diskrétní. Hardwarová implementace (ve formátu [IEEE 754](https://en.wikipedia.org/wiki/Double-precision_floating-point_format)) reprezentuje pouze vybrané body na číselné ose.
