...
-- alias a plná kvalifikace
select p.name, w.date, w.temp_lo from weather w join place p on w.id_place=p.id;

-- výběr výsledků podle podmínky
select p.name, w.date, w.temp_lo from weather w join place p on w.id_place=p.id where p.name='Praha' and 0<w.temp_lo;

-- alias jména sloupce, použit v setřídění
select p.name as místo, w.date, w.temp_lo from weather w join place p on w.id_place=p.id order by místo;
...
-- pracujeme jen se jmény míst, všechny záznamy
...
select distinct p.name as místo from weather w join place p on w.id_place=p.id where místo like '%a%'; -- místo: chyba
-- takto (WTF?)
select distinct p.name as místo from weather w join place p on w.id_place=p.id where p.name like '%a%' order by místo desc;
```
...
Takto realizováno v původní verzi SQL, jako výběr z kartézského součinu:
...
Pravé vnější spojení nic nepřidá, protože existence místa pro každý záznam o počasí je zaručena FK/PK:

```sql
select p.name, w.date, w.temp_hi from place p right outer join weather w on p.id = w.id_place;
```

Když ale vyměníme pořadí tabulek:
...
Typické použití: hierarchie, stromové struktury.
...
Příklad: místa, kdy v prvním místě byl menší rozsah teplot než v druhém:
...
````sql
select id_place from weather where temp_lo = max(temp_lo); -- XX```
...
Pole použitá v `select` musí být buď agregovaná nebo uvedená v `group by`.
