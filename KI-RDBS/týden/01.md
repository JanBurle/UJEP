...
-- validate data
create or replace function  -- must not be empty nor start or end with spaces
is_name(val text) returns boolean as $$ begin
...
Zapoměli jsme na malá/velká písmena názvů. Nejsou podstatná. Opravíme pomocí `citext`. Současně přestaneme používat `varchar`. Postgres má nativní znakový typ [`text`](https://www.postgresql.org/docs/current/datatype-character.html).
